...
const serialize = (
	...
	const combinedStyle = `${fontFamilyStyle} ${colorStyle}  word-wrap: break-word; overflow-wrap: break-word; white-space: normal; `
	const ulStyle =
	...
	const aStyle = `${fontFamilyStyle} text-decoration: none; color: var(${
		theme === 'dark' ? '--primary-button' : '--golden-background'
	}) !important`
	...
	children.forEach(node => {
		if (Text.isText(node)) {
			...
			switch (node.type) {
				case 'h1':
					html += `<h1 style="${combinedStyle}">${serialize(
						node.children,
						color
					)}</h1>`
					break
				case 'h2':
					html += `<h2 style="${combinedStyle}">${serialize(
						node.children,
						color
					)}</h2>`
					break
				case 'h3':
					html += `<h3 style="${combinedStyle}">${serialize(
						node.children,
						color
					)}</h3>`
					break
				case 'h4':
					html += `<h4 style="${combinedStyle}">${serialize(
						node.children,
						color
					)}</h4>`
					break
				case 'h5':
					html += `<h5 style="${combinedStyle}">${serialize(
						node.children,
						color
					)}</h5>`
					break
				case 'h6':
					html += `<h6 style="${combinedStyle}">${serialize(
						node.children,
						color
					)}</h6>`
					break
				case 'quote':
					...
					break
				case 'ul':
					html += `<ul style="${ulStyle}">${serialize(
						node.children,
						color
					)}</ul>`
					break
				case 'ol':
					html += `<ol style="${combinedStyle}">${serialize(
						node.children,
						color
					)}</ol>`
					break
				...
				case 'upload':
					...
					break
				default:
					html += `<p style="${combinedStyle}">${serialize(
						node.children,
						color
					)}</p>`
					break
